...
const FixedVoiceControl: React.FC<{
  availableCommands: string[];
  onCommandMatch: (cmd: string) => void;
  onNoMatch: (cmd: string) => void;
  setToastMessage: (msg: string) => void;
}> = ({ availableCommands, onCommandMatch, onNoMatch, setToastMessage }) => {
  return createPortal(
    <div className="fixed bottom-5 left-5 z-50 h-20 flex items-center">
      <VoiceControl
    ...
    <div className="flex items-center bg-white rounded-lg shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 focus-within:ring-2 focus-within:ring-sky-500 dark:bg-slate-700">
      ...
      <div className="flex items-center pr-1">
        ...
        <button
          ...
          className="p-2 rounded-full text-amber-400 hover:bg-amber-50 dark:hover:bg-slate-600"
          aria-label={isFavorite ? `Unfavorite ${task}` : `Favorite ${task}`}
        >
          <StarIcon filled={isFavorite} />
        </button>
        <button
          onClick={handleDelete}
          className="p-2 rounded-full text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-slate-600 dark:hover:text-red-400"
...
export const TaskSelection: React.FC<TaskSelectionProps> = ({
  ...
  const handleDeleteTask = (taskToDelete: string) => {
    if (
      window.confirm(
        `Are you sure you want to delete "${taskToDelete}"? This cannot be undone.`
      )
    ) {
  ...
  const renderTaskSection = (taskList: string[], title: string) => {
    ...
    return (
      <div className="mb-10">
        <h3 className="text-2xl font-bold text-slate-700 mb-4 dark:text-slate-200">
          {title}
        </h3>
      ...
      <main className="container mx-auto max-w-4xl px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">
            Create a custom guide, or select from the list below.
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg mb-10 dark:bg-slate-800">
          ...
          <p className="mt-1 text-slate-600 mb-4 dark:text-slate-400">
            Describe a task, and AI will create a step-by-step guide with images
            for you.
          </p>
        ...
        {(favoriteTasks.length > 0 || recentTasks.length > 0) && (
          <div className="text-center mb-6">
            <div className="inline-block relative w-full">
              ...
              <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-100 dark:bg-slate-900 px-4 text-slate-500 font-medium dark:text-slate-400">
                ALL TASKS
              </span>
            </div>
          </div>
        ...
        {tasks.length > 0 ? (
          ...
          <div className="text-center p-8 bg-white rounded-lg shadow dark:bg-slate-800">
            <p className="text-slate-500 dark:text-slate-400">
              No tasks here. Try creating a custom guide!
            </p>
          </div>
        )}
      </main>
