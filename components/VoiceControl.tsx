...
export const VoiceControl: React.FC<VoiceControlProps> = ({
  ...
  useEffect(() => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      const matchedCommand = await findMatchingOption(
        transcript,
        availableCommands
      );
  ...
  return (
    <button
      ...
      className={`h-16 w-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-400 ${
