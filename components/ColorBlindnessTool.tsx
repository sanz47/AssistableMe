...
export const ColorBlindnessTool: React.FC<{ onGoBack: () => void; totalPoints: number; }> = ({ onGoBack, totalPoints }) => {
    ...
    const handleCommandMatch = (command: string) => {
        setToastMessage(`Command: ${command}`);
        if (command === 'Upload Image') {
            fileInputRef.current?.click();
        } else if (command === 'Generate Correction') {
            handleGenerate();
        } else if (command === 'Go Back') {
            onGoBack();
        } else {
            const typeKey = Object.keys(COLOR_BLINDNESS_TYPES).find(key => COLOR_BLINDNESS_TYPES[key as ColorBlindnessType] === command);
            if (typeKey) {
                setSelectedType(typeKey as ColorBlindnessType);
            }
        }
    };
    ...
    return (
        <>
                 ...
                 <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg transition-colors duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end mb-6">
                        <div className="md:col-span-1">
                            ...
                            <input ref={fileInputRef} id="image-upload" type="file" accept="image/*" onChange={handleFileChange} className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-sky-50 dark:file:bg-sky-900/50 file:text-sky-700 dark:file:text-sky-300 hover:file:bg-sky-100 dark:hover:file:bg-sky-800/50 transition-colors"/>
                        </div>
                        <div className="md:col-span-1">
                            <label htmlFor="colorblind-type" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 w-full">Color Vision Type</label>
            ...
            <VoiceControl
                availableCommands={[...Object.values(COLOR_BLINDNESS_TYPES), 'Upload Image', 'Generate Correction', 'Go Back']}
